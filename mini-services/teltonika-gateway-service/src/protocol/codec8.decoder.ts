/**
 * Teltonika Codec 8 Decoder (Standard Codec ID 0x08)
 */
...
export function decodeCodec8(buffer: Buffer): RawTeltonikaPacket | null {
  ...
  for (let r = 0; r < recordCount1; r++) {
    ...
    const { io, nextOffset: ioNext } = decodeIoElements(buffer, offset, false);
    offset = ioNext;
