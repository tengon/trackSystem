/**
 * Teltonika Codec 16 Decoder (Codec ID 0x10 - Specialized CAN telemetry)
 */
...
export function decodeCodec16(buffer: Buffer): RawTeltonikaPacket | null {
  ...
  for (let r = 0; r < recordCount1; r++) {
    ...
    const { io, nextOffset: ioNext } = decodeIoElements(buffer, offset, true);
    offset = ioNext;
