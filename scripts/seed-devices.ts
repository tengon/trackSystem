...
async function seed() {
  ...
  for (const u of demoUsers) {
    await db.user.upsert({
      where: { email: u.email },
      update: { password: u.password, role: u.role, active: true },
      create: u,
    });
  }

  const allUsers = await db.user.findMany();
  console.log('Ensured', allUsers.length, 'users in database');

  const existingDeviceCount = await db.device.count();
  ...
  console.log('Seeded successfully with', totalDevices, 'devices.');
}
